...
LOGGER = logging.getLogger("phase2_1.report")
...
DEFAULT_TARGETS_CSV = Path(r"D:\Masters\RQ5\Codex_Chatgpt_drying\outputs\phase2.1\phase2_1_targets.csv")
DEFAULT_OUT_DIR = Path(r"D:\Masters\RQ5\Codex_Chatgpt_drying\outputs\phase2.1\diagnostics")
...
def main(argv: List[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Phase-2.1 diagnostics")
    ...
    meta, _ = load_models(args.models_dir)
    ...
    for fold_id, (train_idx, val_idx) in enumerate(folds, 1):
        ...
        for position, idx in enumerate(val_idx):
            ...
            join_time = float(row.get("tR_start", row.get("tL_end", 0.0)))
