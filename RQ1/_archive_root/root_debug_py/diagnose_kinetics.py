...
# This script is in RQ1/ folder, so we need to add RQ1/src to path
...
print(f"\nMethod 1 (keff_from_state): K_eff = {k_eff_1:.6e} s^-1")

# Method 2: K_eff_from_T_RH (simple formula)
k_eff_2 = K_eff_from_T_RH(T_in_C, RH_in_frac, cfg)
print(f"Method 2 (K_eff_from_T_RH): K_eff = {k_eff_2:.6e} s^-1")
...
print(f"\nRatio: {24.3 / 15.7:.2f}x slower")
print(f"K_eff should be: {k_eff_1 * (24.3 / 15.7):.6e} s^-1 to match baseline")

# Test with different RH values
print("\n" + "="*70)
print("K_eff vs RH (at T=50°C)")
print("="*70)
print(f"\n{'RH %':>6} | {'K_eff (s^-1)':>15} | {'Time (h)':>10}")
print("-"*40)
