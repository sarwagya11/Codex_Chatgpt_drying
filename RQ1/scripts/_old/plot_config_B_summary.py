"""Config B (solar + HP, r=0) summary across 4 sites x 5 weathers x N areas.

Panels:
  1. SEC bars at A_solar=10 m^2 (with SMER labels)
  2. Drying time bars at A_solar=10 m^2
  3. Q_solar useful (delivered to chamber) vs wasted (dumped > T_set) stacked
...
AREAS = [2, 5, 10, 15, 20]
HEADLINE_AREA = 10
CFG_B = ROOT / "outputs" / "config_B"


def load_run(site: str, weather: str, area: int):
    fname = f"Ac_{area}m2.csv"
    p = CFG_B / site / (fname if weather == "annual" else f"{weather}/{fname}")
    if not p.exists():
        return None
    d = pd.read_csv(p).iloc[-1]
    q_useful = float(d.get("Q_solar_cum_kWh", 0.0))
...
def main():
    ...
    qdel = [r["q_delivered"] if r else np.nan for r in headline]
    qwas = [r["q_wasted"]    if r else np.nan for r in headline]

    # --- Panel 1: SEC + SMER labels ---
    ax = axes[0]
    bars = ax.bar(x, sec, 0.7, color="#2ca02c", edgecolor="black", linewidth=0.4)
    ...
    ax.set_title(f"Config B (solar + HP, r=0) — A_solar = {HEADLINE_AREA} m²")
    ...
    # --- Panel 3: Q_solar delivered vs wasted (stacked) ---
    ax = axes[2]
    ax.bar(x, qdel, 0.7, color="#ff7f0e", edgecolor="black", linewidth=0.4,
           label="Q_solar delivered (to chamber)")
    ax.bar(x, qwas, 0.7, bottom=qdel, color="#7f7f7f", edgecolor="black", linewidth=0.4,
           label="Q_solar wasted (> T_set)")
    for xi, (d, w) in enumerate(zip(qdel, qwas)):
        if not np.isnan(d):
            tot = d + w
            pct_w = 100.0 * w / tot if tot > 0 else 0.0
            ax.text(xi, tot + 0.6, f"{d:.1f}+{w:.1f}\n({pct_w:.0f}% lost)",
    ...
    fig.tight_layout()
    out = ROOT / "plots" / "config_B_summary.png"
    ...
    out_csv = ROOT / "outputs" / "config_B_sweep_summary.csv"
    df.to_csv(out_csv, index=False)
    print(f"Saved {out_csv}")

    # Headline table at HEADLINE_AREA
    head = df[df["A_solar_m2"] == HEADLINE_AREA][
        ["site", "weather", "sec", "smer", "time_h", "q_delivered", "q_wasted"]
    ]
    out_head = ROOT / "outputs" / f"config_B_summary_A{HEADLINE_AREA}.csv"
