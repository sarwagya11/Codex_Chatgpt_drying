...
Against this backdrop, this study has been undertaken to first check the performance of HPD, SAHPD, and SAHPD+HRU configurations under Nepali climate, and on that basis to identify the best placement of the solar collector within heat-pump-based and heat-pump-plus-heat-recovery-exchanger-based drying configurations for apple drying. The heat-recovery exchanger, where present, is consistently coupled to the dryer exhaust to recover sensible enthalpy before venting, while the solar collector is repositioned across the plausible integration points on the conditioning side of the air loop; the resulting ten air-path configurations (Section 4.3, Table 1) are evaluated open-loop at recirculation fraction r = 0, so that the configurations are distinguished solely by their topology (where the solar collector, the HRX, and the exhaust stream connect into the air loop) and not by an additional recirculation-fraction degree of freedom. A common drying-air set point of 45 °C and a fresh apple load of 22.5 kg are imposed across every configuration, and four Nepali sites at 72 to 1820 m elevation (Biratnagar in the humid-subtropical Terai, Kathmandu in the temperate mid-hill capital, Dhulikhel on the temperate mid-hill margin, and Taplejung in the cool eastern highland) are simulated under site-specific PVGIS-SARAH3 typical-meteorological-year hourly weather [PVGIS-JRC 2024, ledger #63]; the four sites span a climate envelope spanning 72 to 1820 m elevation across Nepal's tropical-subtropical Terai, temperate mid-hill, and sub-alpine highland bands, so that the SEC ranking exposed by the simulation is robust against the country's ambient-humidity and ambient-irradiance envelope. Annual and seasonal (autumn, winter, spring, summer) subsets are retained to expose the winter-irradiance and pre-monsoon humidity sensitivity of each configuration; the monsoon period (July-September) is excluded from the seasonal partition because the principal apple harvest and small-scale drying activity in Nepal fall outside it. The contributions of this work are: a side-by-side ranking of the solar-placement variants under a five-parameter drying-rate law calibrated on the apple thin-layer dataset of Royen et al. [ledger #40], with the fit protocol, calibration band, and parameter uncertainty bounds reported in §3.4 and §5.6; a quantification of how the energy advantage of solar pre-conditioning varies across Nepal's tropical-Terai, temperate-mid-hill and sub-alpine-highland climate bands; and a design recommendation, transferable across the 72 to 1820 m climate envelope, that nominates a preferred topology and a collector-area range for apple drying at the 45 °C set point characteristic of Nepali installations.
...
- (vii) The air loop carries two blowers in every heat-pump-equipped configuration: a main chamber blower that drives the conditioning path from ambient to chamber-outlet, and a second blower that drives the evaporator source stream. The electric-resistance baseline carries the main blower only. The two blowers are mechanically independent and their per-step shaft powers are summed into the total fan power W_fan (Section 3.10).
...
| Apple slice apparent density | ρ_s | 600 | kg/m³ |
...
| Condenser pressure drop | ΔP_cond | 80 | Pa |
| Evaporator pressure drop | ΔP_evap | 80 | Pa |
| HRX per-side pressure drop | ΔP_HRX,side | 50 | Pa |
| Collector pressure drop | ΔP_solar | 50 | Pa |
| Duct pressure drop | ΔP_duct | 50 | Pa |
| Per-bend dynamic-pressure factor | K_bend | 2.0 | – |
...
### 3.8 Iterative evaporator sizing (D2, E2, E3)

In configurations D2, E2, and E3 the evaporator source stream is the cooled exhaust leaving the HRX hot side. The mass flow on the evaporator side is therefore fixed at the main-loop dry-air mass flow ṁ_da, but the evaporator duty Q_evap (Eq. 8) generally requires more capacity than the cooled exhaust alone can supply, particularly during the constant-rate drying period when the exhaust dewpoint is near the chamber set point. A second ambient stream of mass flow ṁ_amb,extra is therefore mixed into the evaporator inlet to make up the shortfall; ṁ_amb,extra is found at every timestep by a fixed-point iteration on the evaporator energy balance.
...
The path-summed pressure drop ΔP_j on each blower combines the per-component values in Table 2 with a per-bend dynamic-pressure loss K_bend · q (q = ½ · ρ_air · v², ρ_air at T_set), with N_bend = 4 in the present geometry (two 180° serpentine returns). The shaft power of blower j follows the standard fan-affinity relation,
...
| B1_open      | r = 0    | Amb → Solar → Cond → Chamber                                            | Exh (+ dynamic Amb supplement)                      | Fixed T_cond                     | Solar + HP                |
| B2_open      | r = 0    | Amb → Cond(var T_cond) → Solar → Chamber                                | Exh (+ dynamic Amb supplement)                      | Variable T_cond, solar-priority  | Solar + HP                |
...
| D2           | r = 0    | Amb → HRX-cold → Cond → Chamber; Exh → HRX-hot → Exh_cooled             | Exh_cooled (+ dynamic Amb supplement)               | Fixed T_cond                     | HRX + HP                  |
...
A common drying-air set-point, fresh load, and tray geometry are imposed across all ten configurations in Table 1; the fixed chamber and product parameters are collected in Table 2. The HRX effectiveness ε_HRX = 0.70 and the heat-pump cycle parameters are also held constant across the configurations that contain those components.

### 4.4 Simulation matrix

Each of the ten configurations is run at all four sites under the annual TMY baseline at the baseline collector area A_c = 10 m² (40 cases) and again at each of the four seasonal windows at the same baseline area (160 cases). For the two best-performing solar variants identified by the annual baseline, the collector area is additionally swept over A_c ∈ {2, 4, 6, 8, 10, 12, 15} m² at all four sites under the annual TMY (56 cases). The complete matrix comprises 256 simulations.
