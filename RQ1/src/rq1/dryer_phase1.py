...
def run_phase1_simulation(cfg: SimulationConfig) -> Phase1Result:
    ...
    * Air passes through n_trays in series; each tray removes dm_w_tray with latent cooling.
    ...
    * Total dm_w in a step = sum over trays; global X_db is tray-average.
    ...
    for step_idx, row in enumerate(amb_df.itertuples(index=False)):
        T_amb_C = float(row.T_amb_C)  # type: ignore
        RH_amb_frac = float(row.RH_amb_pct) / 100.0  # type: ignore
        ...
        for i in range(n_trays):
            ...
            dm_w_air_max_kg = (
                ...
                if cfg.kinetics.enable_air_limit  # type: ignore
                else float("inf")
            )
        ...
        record = {
            "time_s": time_s,
            "T_amb_C": T_amb_C,
            "RH_amb_pct": float(row.RH_amb_pct),  # type: ignore
            "T_mix_C": T_mix_C,
