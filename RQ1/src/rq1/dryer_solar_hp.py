"""Solar-assisted heat pump dryer simulator - ALL 5 CONFIGS.
...
- Config C: Solar-assisted HP evaporator (higher COP from warmer evap source)
- Config D: Solar-only (passive, variable temperature)
- Config E: Solar cascade (solar preheats air AND boosts evaporator)
...
from rq1.heatpump import HeatPumpConfig, size_heat_pump_for_air_heating
...
def simulate_drying_chamber(
    ...
    """Simulate 10-tray drying chamber with series airflow."""
    ...
    for i in range(n_trays):
        ...
        dm_w_kin = compute_dm_w_kinetic_first_order(
            X_db=X_j, X_eq_db=X_eq_db, T_in_C=T_air, RH_in_frac=RH_air,
            dt_s=dt_s, cfg=cfg.kinetics, m_p_dry_kg=m_p_tray, time_s=time_s,
        )
...
def create_record(
    ...
    MR_global = (X_avg - cfg.dryer.X_eq_db) / (cfg.dryer.X0_db - cfg.dryer.X_eq_db) if cfg.dryer.X0_db != cfg.dryer.X_eq_db else 0.0
    ...
    record = {
        ...
        "RH_to_chamber_frac": float(RH_from_T_omega(T_to_chamber_C, omega_to_chamber)),
        "omega_to_chamber": omega_to_chamber,
        "T_exhaust_C": T_tray_out[-1], "RH_exhaust_frac": RH_tray_out[-1],
...
def simulate_config_A_HP_only(cfg: SimulationConfig) -> SolarHPDryerResult:
    """Config A: Heat pump only, 24/7 operation (baseline)."""
    ...
    print(f"[CONFIG A] HP-only baseline, {n_trays} trays, m_da={m_da:.4f} kg/s")
    print(f"[DIAGNOSTIC] Expected m_da ~ 0.119 kg/s for v=1.1 m/s")
    print(f"[DIAGNOSTIC] cfg.dryer.m_da_kg_per_s = {cfg.dryer.m_da_kg_per_s:.4f} kg/s")
    print(f"[DIAGNOSTIC] Ratio: {m_da / 0.119:.2f}x expected")
    ...
    for row in weather_df.itertuples(index=False):
        ...
        hp_result = size_heat_pump_for_air_heating(
            T_air_in_C=T_amb_C, T_air_out_target_C=cfg.dryer.T_set_C,
            m_air_kg_per_s=m_da, T_evap_source_C=T_amb_C, cfg=hp_cfg,
        )
    ...
    if m_w_cum > 0:
        result_df["SEC_elec_kWh_per_kg"] = W_comp_cum_kWh / m_w_cum
...
def simulate_config_B_solar_HP_series(cfg: SimulationConfig) -> SolarHPDryerResult:
    ...
    print(f"[CONFIG B] Solar+HP series, {n_trays} trays, A_solar={cfg.solar.area_m2:.1f}m2")
...
def simulate_config_C_solar_HP_evap(cfg: SimulationConfig) -> SolarHPDryerResult:
    ...
    print(f"[CONFIG C] Solar-assisted HP evaporator, {n_trays} trays, A_solar={cfg.solar.area_m2:.1f}m2")
    ...
    for row in weather_df.itertuples(index=False):
        ...
        T_evap_source = solar_state.T_out_C
        ...
        record = create_record(
            time_s, T_amb_C, row.RH_amb_pct, G_solar,
            solar_state.Q_useful_kW, T_evap_source, solar_state.eta_collector,
            hp_result, T_to_chamber_C, omega_to_chamber,
            T_tray_out, RH_tray_out, X_trays, MR_trays, dm_w_trays,
            m_w_cum, W_comp_cum_kWh, Q_cond_cum_kWh, Q_solar_cum_kWh, cfg
        )
    ...
    return SolarHPDryerResult(
        times_s=result_df["time_s"], df=result_df, config_type="CONFIG_C",
        converged=all(X <= cfg.dryer.X_final_db + 1e-6 for X in X_trays),
        final_message=final_msg,
    )
...
def simulate_config_D_solar_only(cfg: SimulationConfig) -> SolarHPDryerResult:
    """Config D: Solar only, no heat pump. Limited to 72 hours."""
    m_da, dt_s, X_trays, MR_trays = setup_simulation(cfg)
    n_trays = cfg.dryer.n_trays
    ...
    print(f"[CONFIG D] Solar-only, {n_trays} trays, A_solar={cfg.solar.area_m2:.1f}m2")
    print(f"[CONFIG D] Max simulation time: {max_time_s/3600:.0f}h")
    ...
    for row in weather_df.itertuples(index=False):
        ...
        T_to_chamber_C = solar_state.T_out_C
        omega_to_chamber = omega_amb
        h_to_chamber = float(moist_air_enthalpy_kJ_per_kg(T_to_chamber_C, omega_to_chamber))
        ...
        record = create_record(
            time_s, T_amb_C, row.RH_amb_pct, G_solar,
            solar_state.Q_useful_kW, T_to_chamber_C, solar_state.eta_collector,
            None, T_to_chamber_C, omega_to_chamber,
            T_tray_out, RH_tray_out, X_trays, MR_trays, dm_w_trays,
            m_w_cum, W_comp_cum_kWh, Q_cond_cum_kWh, Q_solar_cum_kWh, cfg
        )
        records.append(record)
        ...
        if time_s >= max_time_s:
            final_msg = f"Config D time limit ({max_time_s/3600:.0f}h) reached"
            break
    else:
        final_msg = f"Weather data exhausted at t={time_s/3600:.1f}h"
    
    result_df = pd.DataFrame.from_records(records)
    result_df["SEC_elec_kWh_per_kg"] = 0.0
    
    return SolarHPDryerResult(
        times_s=result_df["time_s"], df=result_df, config_type="CONFIG_D",
        converged=all(X <= cfg.dryer.X_final_db + 1e-6 for X in X_trays),
        final_message=final_msg,
    )
...
def simulate_config_E_cascade(cfg: SimulationConfig) -> SolarHPDryerResult:
    ...
    print(f"[CONFIG E] Solar cascade, {n_trays} trays, A_solar={cfg.solar.area_m2:.1f}m2")
    ...
    for row in weather_df.itertuples(index=False):
        ...
        if T_after_solar >= cfg.dryer.T_set_C:
            ...
            hp_result = size_heat_pump_for_air_heating(
                T_air_in_C=T_after_solar, T_air_out_target_C=cfg.dryer.T_set_C,
                m_air_kg_per_s=m_da, T_evap_source_C=T_after_solar, cfg=hp_cfg,
            )
            W_comp_kW = hp_result.W_comp_kW
            Q_cond_kW = hp_result.Q_cond_kW
            T_to_chamber_C = cfg.dryer.T_set_C
    ...
    return SolarHPDryerResult(
        times_s=result_df["time_s"], df=result_df, config_type="CONFIG_E",
        converged=all(X <= cfg.dryer.X_final_db + 1e-6 for X in X_trays),
        final_message=final_msg,
    )
