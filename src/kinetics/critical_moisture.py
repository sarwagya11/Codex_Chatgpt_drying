...
def _enforce_strictly_increasing(
...
def detect_critical_moisture(
    ...
    *,
    ...
    if time.size < 2:
        if debug:
            print("[detect] abort: <2 samples after finite filter")
        return None

    time, values = _enforce_strictly_increasing(time, values)
    if time.size < 2:
        if debug:
            print("[detect] abort: <2 samples after enforcing increasing time")
        return None
    ...
    if time.size < 2:
        if debug:
            print("[detect] abort: <2 samples after dt>0 filter")
        return None
    ...
    if m == 0:
        if debug:
            print("[detect] abort: m=0 after positivity filter")
        return None

    min_leaf = max(8, int(np.ceil(0.08 * m)))
    if m < 2 * min_leaf:
        if debug:
            print(f"[detect] abort: m={m} < 2*min_leaf={2*min_leaf}")
        return None
    ...
    if candidate_start >= candidate_stop:
        if debug:
            print(f"[detect] abort: empty candidate window [{candidate_start},{candidate_stop})")
        return None
    ...
    for k in range(candidate_start, candidate_stop):
        ...
        try:
            a0, b1, b2 = tuple(np.linalg.solve(AtA, Atb))
        except np.linalg.LinAlgError:
            a0, b1, b2 = tuple(np.linalg.pinv(AtA) @ Atb)

        # Constraint: b2 must be non-positive. If b2 >= 0, refit without it.
        if b2 >= 0.0:
            b2 = 0.0
            ...
            Atb_refit = A_refit.T @ b
            AtA_refit[1, 1] += lambda_const

            try:
                a0, b1 = tuple(np.linalg.solve(AtA_refit, Atb_refit))
            except np.linalg.LinAlgError:
                a0, b1 = tuple(np.linalg.pinv(AtA_refit) @ Atb_refit)
        ...
        if bic < best_bic:
            best_bic = bic
            best_params = (k, float(a0), float(b1), float(b2), float(Xc))

    if best_params is None:
        if debug:
            print("[detect] abort: no solvable candidate produced a finite BIC")
        return None
    ...
    try:
        alpha, beta = tuple(np.linalg.solve(AtA_null, Atb_null))
    except np.linalg.LinAlgError:
    ...
    if debug:
        print(f"[detect] m={m} min_leaf={min_leaf} candidates=[{candidate_start},{candidate_stop})")
        print(
            f"[detect] best t~{float(midpoint_time[k_star]):.3f} Xc~{Xc_star:.6f} "
            f"delta_bic={delta:.2f} left={left_indices.size} right={right_indices.size}"
        )
    ...
    if delta < acceptance_delta or left_indices.size < min_leaf or right_indices.size < min_leaf:
        if debug:
            ...
            if delta < acceptance_delta:
                reasons.append(f"delta_bic {delta:.2f} < {acceptance_delta:.2f}")
            if left_indices.size < min_leaf:
                reasons.append(f"left {left_indices.size} < {min_leaf}")
            if right_indices.size < min_leaf:
                reasons.append(f"right {right_indices.size} < {min_leaf}")
